...
generate
	for (c = 0; c < 64; c = c + 1) begin: GATED_0
		...
		//GATED_OR GATED_(.CLOCK(clk),.SLEEP_CTRL(0),.RST_N(rst_n),.CLOCK_GATED(gated_clk[c]));
	end
endgenerate
...
//GATED_OR GATED_out_cnt(.CLOCK(clk),.SLEEP_CTRL(current_state!=OUTPUT),.RST_N(rst_n),.CLOCK_GATED(clk_out_cnt));
////GATED_OR GATED_out_cnt(.CLOCK(clk),.SLEEP_CTRL(zero),.RST_N(rst_n),.CLOCK_GATED(clk_out_cnt));
//
////current_pos
//wire clk_current_pos;
////GATED_OR GATED_current_pos(.CLOCK(clk),.SLEEP_CTRL(!(current_state==IDLE&&current_pos!=27||done&&current_op_idx!=15&&current_op>4&&(current_state==INPUT||input_cnt==64))),.RST_N(rst_n),.CLOCK_GATED(clk_current_pos));
//GATED_OR GATED_current_pos(.CLOCK(clk),.SLEEP_CTRL(zero),.RST_N(rst_n),.CLOCK_GATED(clk_current_pos));
//
////current_op_idx
//wire clk_current_op_idx;
////GATED_OR GATED_current_op_idx(.CLOCK(clk),.SLEEP_CTRL(!(done&&current_op_idx!=15||current_state==IDLE)),.RST_N(rst_n),.CLOCK_GATED(clk_current_op_idx));
//GATED_OR GATED_current_op_idx(.CLOCK(clk),.SLEEP_CTRL(zero),.RST_N(rst_n),.CLOCK_GATED(clk_current_op_idx));
//
//
////input_cnt
//wire clk_input_cnt;
////GATED_OR GATED_input_cnt(.CLOCK(clk),.SLEEP_CTRL(!(out_cnt==0)),.RST_N(rst_n),.CLOCK_GATED(clk_input_cnt));
//GATED_OR GATED_input_cnt(.CLOCK(clk),.SLEEP_CTRL(zero),.RST_N(rst_n),.CLOCK_GATED(clk_input_cnt));
//
////cs
//
////GATED_OR GATED_cs(.CLOCK(clk),.SLEEP_CTRL(!(current_state==IDLE||current_state==INPUT||(current_op_idx==15&&input_cnt==64&&current_state!=OUTPUT)||out_cnt==15)),.RST_N(rst_n),.CLOCK_GATED(clk_cs));
//GATED_OR GATED_cs(.CLOCK(clk),.SLEEP_CTRL(zero),.RST_N(rst_n),.CLOCK_GATED(clk_cs));
